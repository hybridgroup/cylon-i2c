...
Bme280.prototype.start = function(callback) {
  ...
  this.connection.i2cWrite(
      this.address,
      Bme280.BME280_REGISTER_CONTROL,
      [ 0x3F ],
      function(err) {
    if (err) {
      callback(err, null);
 ...
 *
 * Since temperature is also calculated to determine pressure,
 * it returns the temperature as well.
 *
...
Bme280.prototype.readCalibrationData = function(callback) {
  ...
  this.connection.i2cRead(this.address,                  // Device i2c addr
                          ...
                          7,             // read 1 reg of 2 bytes, 6 of 1 byte.
                          function(err, d) {
    if (err) {
      callback(err, null);
    } else {
...
var toHexString = function(num, len) {
  ...
  return "0x" + lpad((num & getMask(num)).toString(16).trim().toUpperCase(),
                     l, "0");
};
...
var readTemp = function(self, callback) {
  self.connection.i2cRead(
      self.address,
      Bme280.BME280_REGISTER_TEMP_DATA,
      3,
      function(error, d) {
        if (error) {
          callback(error, null);
...
Bme280.prototype.readRawTemp = function(callback) {
  ...
  this.connection.i2cWrite(
      self.address,
      Bme280.BME280_REGISTER_CONTROL_HUM,
      [ meas ],
      function(err) {
    if (err) {
      callback(err, null);
...
Bme280.prototype.readRawPressure = function(callback) {
  ...
  self.connection.i2cRead(
      self.address,
      Bme280.BME280_REGISTER_PRESSURE_DATA,
      3,
      function(error, d) {
      if (error) {
        callback(error, null);
...
Bme280.prototype.readRawHumidity = function(callback) {
  ...
  self.connection.i2cRead(
      self.address,
      Bme280.BME280_REGISTER_HUMIDITY_DATA,
      2,
      function(error, d) {
      if (error) {
        callback(error, null);
        ...
        if (verbose) {
          ...
          console.log("Raw hum:" + hum + ", msb:",
                      toHexString(msb) + " lsb:" + toHexString(lsb));
        }
        callback(null, hum);
      }
    }
...
Bme280.prototype.calculateTemperature = function(rawTemp) {
  ...
  var2 = ((UT / 131072.0 - this.dig_T1 / 8192.0) *
          (UT / 131072.0 - this.dig_T1 / 8192.0)) * this.dig_T3;
...
Bme280.prototype.calculateHumidity = function(rawHum) {
  ...
  h = (adc - (this.dig_H4 * 64.0 + this.dig_H5 / 16384.8 * h)) *
      (this.dig_H2 / 65536.0 * (1.0 + this.dig_H6 / 67108864.0 * h *
      (1.0 + this.dig_H3 / 67108864.0 * h)));

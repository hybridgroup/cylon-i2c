...
LSM9DS0XM.prototype.getAccel = function(callback) {
  ...
  self.adaptor.i2cRead(self.address, OUT_X_L_A, 6, function(err, data) {
    if (err) {
      callback(err, null);
    } else {
      var result = {};
...
LSM9DS0XM.prototype.getMag = function(callback) {
  ...
  self.adaptor.i2cRead(self.address, OUT_X_L_M, 6, function(err, data) {
    if (err) {
      callback(err, null);
    } else {
      var result = {};
...
LSM9DS0XM.prototype._initAccel = function() {
  ...
  this.adaptor.i2cWrite(this.address, CTRL_REG0_XM, 0x00);
  ...
  this.adaptor.i2cWrite(this.address, CTRL_REG1_XM, 0x57); // 100Hz data rate, x/y/z all enabled
  ...
  SIM - SPI mode selection
    0=4-wire, 1=3-wire                           */
  this.adaptor.i2cWrite(this.address, CTRL_REG2_XM, 0x00); // Set scale to 2g
  
  /* CTRL_REG3_XM is used to set interrupt generators on INT1_XM
  Bits (7-0): P1_BOOT P1_TAP P1_INT1 P1_INT2 P1_INTM P1_DRDYA P1_DRDYM P1_EMPTY
  */
  this.adaptor.i2cWrite(this.address, CTRL_REG3_XM, 0x04); // Accelerometer data ready on INT1_XM (0x04)
};
...
LSM9DS0XM.prototype._initMag = function() {
  ...
  this.adaptor.i2cWrite(this.address, CTRL_REG5_XM, 0x94); // Mag data rate - 100 Hz, enable temperature sensor
  ...
  this.adaptor.i2cWrite(this.address, CTRL_REG6_XM, 0x00); // Mag scale to +/- 2GS
  ...
  this.adaptor.i2cWrite(this.address, CTRL_REG7_XM, 0x00); // Continuous conversion mode
  
  /* CTRL_REG4_XM is used to set interrupt generators on INT2_XM
  Bits (7-0): P2_TAP P2_INT1 P2_INT2 P2_INTM P2_DRDYA P2_DRDYM P2_Overrun P2_WTM
  */
  this.adaptor.i2cWrite(this.address, CTRL_REG4_XM, 0x04); // Magnetometer data ready on INT2_XM (0x08)
  ...
  MIEN - Enable interrupt generation for magnetic data
    0=disable, 1=enable) */
  this.adaptor.i2cWrite(this.address, INT_CTRL_REG_M, 0x09); // Enable interrupts for mag, active-low, push-pull
};

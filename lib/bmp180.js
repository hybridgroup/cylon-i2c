...
Bmp180.prototype.readCoefficients = function(callback) {
    ...
    this.adaptor.i2cRead(this.address, BMP180_REGISTER_CALIBRATION, 22, function(err, data) {
        if (err) {
          callback(err, null);
        } else {
...
Bmp180.prototype.getRawTemp = function(callback) {
  ...
  this.adaptor.i2cWrite(self.address, BMP180_REGISTER_CONTROL, [BMP180_REGISTER_READTEMPCMD], function(err) {
      if (err) {
        callback(err, null);
      } else {
        setTimeout(function() {
          self.adaptor.i2cRead(self.address, BMP180_REGISTER_TEMPDATA, 2, function(err, data) {
            if (err) {
              callback(err, null);
            } else {
...
Bmp180.prototype.getRawPressure = function(mode, callback) {
    ...
    this.adaptor.i2cWrite(self.address, BMP180_REGISTER_CONTROL, [BMP180_REGISTER_READPRESSURECMD], function(err) {
        if (err) {
          callback(err, null);
        } else {
          var waitTime;
          ...
          setTimeout(function() {
            self.adaptor.i2cRead(self.address, BMP180_REGISTER_PRESSUREDATA, 3, function(err, data) {
              if (err) {
                callback(err, null);
              } else {
                var msb = data[0];

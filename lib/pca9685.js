...
PCA9685.prototype.setPWMFreq = function(frequency) {
    ...
    this.connection.i2cRead(this.address, PCA9685_MODE1, 1, function(err, data) {
        var oldmode = data[0];
        var newmode = [(oldmode & 0x7F) | 0x10]; // sleep
        this.connection.i2cWrite(this.address, PCA9685_MODE1, newmode);
        this.connection.i2cWrite(this.address, PCA9685_PRESCALE, Math.floor(prescale));
 ...
 * @publish
 */
PCA9685.prototype.setPWM = function(channel, pulseon, pulseoff) {
    this.connection.i2cWrite(this.address, LED0_ON_L + 4 * channel, [pulseon & 0xFF]);
    this.connection.i2cWrite(this.address, LED0_ON_H + 4 * channel, [pulseon >> 8]);
    this.connection.i2cWrite(this.address, LED0_OFF_L + 4 * channel, [pulseoff & 0xFF]);
    this.connection.i2cWrite(this.address, LED0_OFF_H + 4 * channel, [pulseoff >> 8]);
};
